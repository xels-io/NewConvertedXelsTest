...
namespace Xels.Bitcoin.IntegrationTests.Wallet
{
    public sealed class WalletHistoryIntegrationTests
    {
        ...
        public async Task WalletCanReturnStakingHistoryCorrectlyAsync()
        {
            ...
            // Start 2 nodes
            CoreNode miner = builder.CreateXelsPosNode(network, "history-1-nodeA", configParameters: configParameters).OverrideDateTimeProvider().WithWallet().Start();
            CoreNode syncer = builder.CreateXelsPosNode(network, "history-1-nodeB", configParameters: configParameters).OverrideDateTimeProvider().WithWallet().Start();
        ...
        public async Task MiningWalletCanReturnHistoryCorrectlyAsync()
        {
            using (var builder = NodeBuilder.Create(this))
            {
                ...
                // Start 3 nodes
                CoreNode miner = builder.CreateXelsPosNode(network, "history-1-nodeA").WithWallet().Start();
                CoreNode nodeA = builder.CreateXelsPosNode(network, "history-1-nodeB").WithWallet().Start();
                CoreNode nodeB = builder.CreateXelsPosNode(network, "history-1-nodeC").WithWallet().Start();
        ...
        public async Task WalletCanReturnHistoryCorrectlyToMultipleRecipientsAsync()
        {
            using (var builder = NodeBuilder.Create(this))
            {
                ...
                // Start 4 nodes
                CoreNode miner = builder.CreateXelsPosNode(network, "history-3-nodeA").WithWallet().Start();
                CoreNode nodeA = builder.CreateXelsPosNode(network, "history-3-nodeB").WithWallet().Start();
                CoreNode nodeB = builder.CreateXelsPosNode(network, "history-3-nodeC").WithWallet().Start();
                CoreNode nodeC = builder.CreateXelsPosNode(network, "history-3-nodeD").WithWallet().Start();
