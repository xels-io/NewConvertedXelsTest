    ...
    public class SegWitTests
    {
        ...
        public void SegwitActivatedOnStraxNode()
        {
            using (NodeBuilder builder = NodeBuilder.Create(this))
            {
                ...
                Assert.Equal(ThresholdState.Defined, cache.GetState(xelsNode.FullNode.ChainIndexer.GetHeader(startedHeight - 1), StraxBIP9Deployments.Segwit));
                Assert.Equal(ThresholdState.Started, cache.GetState(xelsNode.FullNode.ChainIndexer.GetHeader(startedHeight), StraxBIP9Deployments.Segwit));
                Assert.Equal(ThresholdState.LockedIn, cache.GetState(xelsNode.FullNode.ChainIndexer.GetHeader(lockedInHeight), StraxBIP9Deployments.Segwit));
                Assert.Equal(ThresholdState.Active, cache.GetState(xelsNode.FullNode.ChainIndexer.GetHeader(activeHeight), StraxBIP9Deployments.Segwit));
        ...
        [Fact]
        public void TestSegwit_AlwaysActivatedOn_XelsNode()
        {
            using (NodeBuilder builder = NodeBuilder.Create(this))
            {
        ...
        public void MineSegwitBlock()
        {
            using (NodeBuilder builder = NodeBuilder.Create(this))
            {
                ...
                CoreNode node = builder.CreateXelsPosNode(KnownNetworks.StraxRegTest).Start();
        ...
        public void StakeSegwitBlock()
        {
            using (NodeBuilder builder = NodeBuilder.Create(this))
            {
                ...
                CoreNode node = builder.CreateXelsPosNode(network).WithWallet().Start();
        ...
        public void CheckSegwitP2PSerialisationForWitnessNode()
        {
            ...
            {
                CoreNode node = builder.CreateXelsPosNode(KnownNetworks.StraxRegTest).Start();
                CoreNode listener = builder.CreateXelsPosNode(KnownNetworks.StraxRegTest).Start();
        ...
        public void CheckSegwitP2PSerialisationForNonWitnessNode()
        {
            using (NodeBuilder builder = NodeBuilder.Create(this))
            {
                ...
                // Explicitly use new & separate instances of XelsRegTest because we modified the BIP9 deployments on one instance.
                CoreNode node = builder.CreateXelsPosNode(network).Start();
                CoreNode listener = builder.CreateXelsPosNode(networkNoBIP9).Start();
        ...
        public void SegwitWalletTransactionBuildingAndPropagationTest()
        {
            using (NodeBuilder builder = NodeBuilder.Create(this))
            {
                CoreNode node = builder.CreateXelsPosNode(KnownNetworks.StraxRegTest).WithWallet().Start();
                CoreNode listener = builder.CreateXelsPosNode(KnownNetworks.StraxRegTest).WithWallet().Start();
